...
int procesarSolicitudDeEsi(t_operacion op_a_realizar, int socket_solicitante) {
	...
	if (op_a_realizar.tipo == GET) {
		...
		fila_tabla_instancias * instancia =(fila_tabla_instancias * ) malloc(sizeof(fila_tabla_instancias));
			seleccionar_instancia(clave, instancia);
		log_debug(logger_coordinador, "usando instancia#%d", instancia->socket_inst);
		if (instancia == NULL) {
