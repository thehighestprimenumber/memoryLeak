...
int await_msg(int socket, Message *msg) {
	...
	//Recibo el contenido
	result_recv = recv(socket, contenido, header->size, MSG_WAITALL); //TODO validar con dexconeccion/ es bloqueante

	//En caso de error al recibir devuelvo -1
	if(result_recv == -1) return -1;

	//En caso que todo salgo correcto relleno el msg y devuelvo 1 para informar la operacion satisfactoria
	// NORMA POSIX 0=ok  ERROR = !0
	msg->header = header;
	msg->contenido = contenido;
	return 1; // orrible -> TODO> definir macros en un .h como constantes de error.
}
